...
class AwesomeService extends PluginContext {
  ...
  async addAwesomeness (request) {
    // A call to this.param() without paramPath will returns the entire request body
    const documentBody = this.param(request);

    // Modify the document body before creation
    const awesomeMessage = this._getRandomAwesomeness();
    documentBody.awesomeness = awesomeMessage;

    // Increment this message count in redis
